...
-   **Correos electrónicos:** [leoianayaglz\@gmail.com](mailto:leoianayaglz@gmail.com){.email} \| [rubimarch201\@gmail.com](mailto:rubimarch201@gmail.com){.email}
...
1.  Normalización de cuentas (rlog, z-score, melted counts)
...
Las cuentas crudas de RNA-seq no son comparables entre muestras porque dependen del tamaño total de la librería y, en el caso de Kallisto, también de la longitud de los transcritos. La **rlog (regularized logarithm)** es la transformación recomendada por DESeq2 cuando el número de muestras es pequeño (n < 30), como en este caso con 6 muestras. Aplica un log2 regularizado que encoge los valores de genes con poca información hacia la media global, estabilizando la varianza sin los artefactos que VST puede introducir con datasets pequeños. Los datos resultantes quedan en una escala apta para PCA, heatmaps y clustering. Se usa `blind = FALSE` porque conocemos el diseño experimental, lo que permite que la transformación tome en cuenta el genotipo durante la estimación de la dispersión.

### 2.1 rlog y Kallisto

```{r rlog-kallisto}
rld_kallisto <- rlog(dds, blind = FALSE)
cat("Dimensiones rlog Kallisto (genes × muestras):", dim(rld_kallisto), "\n")
```
...
cat("Dimensiones rlog STAR (genes × muestras):", dim(rld_star), "\n")
```

### 2.3 Distribución de cuentas normalizadas

El boxplot muestra la distribución de los valores rlog por muestra. Si la normalización fue exitosa, las medianas de todas las cajas deben estar al mismo nivel, indicando que las muestras son comparables entre sí independientemente del tamaño de su librería.

```{r melted-norm-counts, fig.cap="Figura 1. Distribución de cuentas normalizadas (rlog, Kallisto) por muestra. Las medianas alineadas indican que la normalización fue exitosa y las muestras son comparables.", fig.width=8, fig.height=5}
melted_norm_counts <- data.frame(melt(assay(rld_kallisto)))
...
```{r pca-kallisto, fig.cap="Figura 2. PCA de muestras normalizadas con rlog (Kallisto). Cada punto representa una muestra; el color indica el genotipo (azul = WT, rojo = Dp16). Las elipses muestran la dispersión de cada grupo.", fig.width=7, fig.height=5}
mat_k <- assay(rld_kallisto)
...
```{r pca-star, fig.cap="Figura 4. PCA de muestras normalizadas con rlog (STAR). Se esperan los mismos patrones de agrupamiento que en Kallisto si ambos alineadores son concordantes.", fig.width=7, fig.height=5}
mat_s <- assay(rld_star)
pc_s  <- prcomp(t(mat_s))
...
```{r batch-correction, fig.cap="Figura 5. PCA post-corrección de batch effect (Kallisto rlog). Sin batch conocido, la corrección elimina solo la variación residual no explicada por el genotipo, lo que generalmente refuerza la separación entre grupos.", fig.width=7, fig.height=5}
rld_bc    <- rld_kallisto   # copia para no modificar el original
mm_design <- model.matrix(~ genotype, colData(rld_bc))
mat_bc    <- assay(rld_bc)
mat_bc    <- removeBatchEffect(mat_bc, design = mm_design)
assay(rld_bc) <- mat_bc

pcaData_bc    <- plotPCA(rld_bc, intgroup = "genotype", returnData = TRUE)
...
ggplot(pcaData_bc, aes(PC1, PC2, color = genotype, label = name)) +
  ...
  geom_text_repel(size = 3, show.legend = FALSE) +
  scale_color_manual(values = c("WT" = "steelblue", "Dp16" = "firebrick3")) +
  labs(title = "PCA post-corrección de batch (Kallisto rlog)",
...
```{r heatmap-kallisto, fig.cap="Figura 10. Heatmap de los 20 genes con menor padj — Kallisto (rlog, escalado por fila/z-score). La barra de color en la parte superior indica el genotipo de cada muestra. El clustering jerárquico es no supervisado.", fig.width=8, fig.height=7}
top_genes_k <- head(order(res_kallisto$padj, na.last = TRUE), 20)
mat_heat_k  <- assay(rld_kallisto)[top_genes_k, ]
...
```{r heatmap-star, fig.cap="Figura 11. Heatmap de los 20 genes con menor padj — STAR (rlog, escalado por fila). La comparación con el heatmap de Kallisto (Figura 10) permite identificar genes consistentemente significativos en ambos alineadores.", fig.width=8, fig.height=7}
top_genes_s <- head(order(res_star$padj, na.last = TRUE), 20)
mat_heat_s  <- assay(rld_star)[top_genes_s, ]
